  ...
  return (
    ...
    >
      ...
      <div className="flex flex-row gap-1">
        <ActionButton text="يستخدم" onClick={() => props.insertPrompt(props.prompt)} />
        {props.edit && <ActionButton text="يحرر" onClick={props.edit} />}
        {props.addToLocal && <ActionButton text={saved ? 'وأضاف' : 'اضف إليه'} onClick={copyToLocal} />}
        {props.remove && <ActionButton text="يمسح" onClick={props.remove} />}
      </div>
    </div>
  )
}
  ...
  return (
    <form className="flex flex-col gap-2 w-full" onSubmit={onSubmit}>
      <div className="w-full">
        <span className="text-sm font-semibold block mb-1 text-primary-text">عنوان الكلمة السريعة</span>
        <Input className="w-full" name="title" defaultValue={props.initialData.title} />
      </div>
      <div className="w-full">
        <span className="text-sm font-semibold block mb-1 text-primary-text">محتوى كلمة سريعة</span>
        <Textarea className="w-full" name="prompt" defaultValue={props.initialData.prompt} />
      </div>
      <div className="flex flex-row gap-2 mt-1">
        <Button color="primary" className="w-fit" type="submit">يحفظ</Button>
    ...
    <>
      {localPrompts.length ? (
        ...
        <div className="relative block w-full rounded-lg border-2 border-dashed border-gray-300 p-3 text-center text-sm mt-5 text-primary-text">
          ليس لديك كلمة سريعة حتى الآن，الرجاء الإضافة يدويا
        </div>
      )}
      <div className="mt-5">
        {formData ? (
          ...
          <Button onClick={create}>إنشاء كلمات سريعة</Button>
        )}
      </div>
    </>
  )
...
const Prompts = (props: { insertPrompt: (text: string) => void }) => {
  ...
  const tabs = useMemo<Tab[]>(
    () => [
      { name: 'أرشيف السيارات', value: 'local' },
      { name: 'المعجم العام', value: 'network' },
    ],
    [],
  )
